...
async def show_instruction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    instruction_text = (
        ...
        "Без VPN ты попадёшь в локальную тестовую версию — там профиль не определяется, товары могут быть неактуальными.\n"
        "Пользуйся только основной версией через VPN.\n"
        ...
        "5. После оплаты я свяжусь с тобой в течение дня, чтобы согласовать детали и начать работу.\n"
        "Срок выполнения заказа — 1–2 рабочих дня.\n"
        "6. Возврат возможен только в первый день после оплаты, в течение 6 часов.\n"
        "По всем вопросам — возврат, уточнения, статус заказа — пиши в поддержку.\n\n"
